...
(function($) {
    jsPsych["single-stim-2"] = (function() {
	    ...
	    var audio_ended = false;
            ...
            if (!trial.is_html) {
                if(!trial.is_audio){
					...
					if(trial.stimuli_with_text){
						console.log("Stimuli comes with text");
						display_element.append($("<p class='stimuli_text'>" + trial.stimuli_text + "</p>"));
					}
	    ...
	    $("#jspsych-single-stim-stimulus").on("ended", function(){
		audio_ended = true;
	    });
		...
		if(audio_ended){
		        ...
		        if (flag) {
		            ...
		            console.log("Key " + key_press + " Pressed and Recorded.")
