    ...
    jsPsych["single-stim"] = (function() {
        ...
        plugin.create = function(params) {
            ...
            for (var i = 0; i < trials.length; i++) {
                ...
                trials[i].is_audio = (typeof params.is_audio === 'undefined') ? false : params.is_audio;
        ...
        plugin.trial = function(display_element, block, trial, part) {
			...
			var startTime = performance.now(); // High-res!
            var endTime = -1;
	    ...
	    var audio_left = trial.timing_stim;

	    var audio_cd = function(){
		audio_left -= 1000;
		//console.log(audio_left);
		$("#timer_span").text(parseInt(audio_left / 1000));
		if(audio_left > 0){
			setTimeout(audio_cd, 1000);
		}
            }
		...
		display_element.append("Audio: <span id='timer_span'>"+(trial.timing_stim / 1000)+"</span>s remaining.");
		setTimeout(audio_cd, 1000);
	    }
            ...
            var cont_function = function() {
                ...
                console.log("Trial Data: " + trial_data["rt"])
            ...
            var resp_func = function(e) {
                ...
                if (flag) {
					...
					endTime = performance.now(); // Hi-res!

                    console.log("Key Pressed and Recorded.")
            ...
            if (trial.timing_stim > 0) {
                setTimeout(function() {
                    if (!trial_complete) {
                        $('#jspsych-single-stim-stimulus').css('display', 'none');
                        $(document).unbind('keydown', resp_func);
						display_element.html('');
                        block.next()
                    }
